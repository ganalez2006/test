	...
	if (isset($url) && !empty($url)) {
		...
		function get_elemnt($element) {
			...
			foreach ($element->display_resources as $key => $value) {
				...
				echo '<li><a href="'.$value->src.'" target="_blank">Media Preview | ' . $text . ' | '.$value->config_width.'x'.$value->config_height.'px</a></li>';
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-158112592-1"></script>
<script>
window.dataLayer = window.dataLayer || [];
function gtag(){dataLayer.push(arguments);}
gtag('js', new Date());
gtag('config', 'UA-158112592-1');
</script>
